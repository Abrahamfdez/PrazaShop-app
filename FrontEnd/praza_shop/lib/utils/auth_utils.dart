...
/// Resultado del registro que incluye el rol detectado y el id de usuario
/// (cuando se puede extraer del token).
class RegisterResult {
  final Role role;
  final int? userId;
  RegisterResult({required this.role, this.userId});
}
...
class AuthUtils {
  ...
  static Future<Role> performLoginAndGetInfo(ApiService api, String email, String password) async {
  ...
  /// Registra un nuevo usuario (cliente o negocio) y devuelve el `UserRole` junto
  /// con el `userId` extraído del token cuando esté disponible.
  ///
  ...
  static Future<RegisterResult> registerAndGetRole(ApiService api, Map<String, dynamic> body) async {
    ...
    return RegisterResult(role: role, userId: userId);
  }
}
